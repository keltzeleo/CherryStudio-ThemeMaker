...
2. **点哪改哪。** 预览里的每个元素——气泡、侧边栏、表格、代码块、链接——点击后都会弹出浮动的颜色选择器。拖动那一颗主色球，思考框文字、激活态、发光效果都会自动跟着变；结构性色块会朝莫兰迪色系做去饱和处理，保持柔和，不会变成饱和度过高的主色克隆。
...
- **点哪改哪,改的是真实界面** —— 预览里的每个元素(气泡、侧边栏、表格、代码块、链接)点击后都会弹出自己的颜色选择器,不需要在一堆抽象标签和界面之间来回对照。
- **主色优先的衍生机制** —— 一颗主色球驱动整体基调,激活态、发光效果、思考框文字都会自动跟随。结构性色块(思考框、表头、引用、代码块名背景)会朝莫兰迪色系去饱和,而不是变成饱和度过高的主色克隆。
- **明暗双模式同步** —— 默认开启:改一个模式,另一个自动换算跟随;也支持单次修改时关闭同步,让两种模式刻意分道扬镳。
- **撤销 / 重做** —— 完整历史记录,`⌘Z` / `⌘⇧Z`。
- **非破坏性草稿** —— 原地编辑已保存的预设,或将其分叉成新的浮动草稿(`<名字> v2`),在你决定保留之前不会动到原始预设。
- **键盘无障碍** —— 预设支持 Tab 聚焦、`Space`/`Enter` 选中,无需鼠标。
...
每个预设都单独调校了明暗两种版本。dock 里还放了作者自用的几个个人预设——想用的话可以直接分叉，或者从零开始也一样方便。
